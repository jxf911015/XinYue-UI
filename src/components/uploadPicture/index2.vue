    ...
    <el-row :gutter="30">
                ...
                <el-tree ref="tree" :data="treeData2" :filter-node-method="filterNode" :props="defaultProps" highlight-current>
                  <div slot-scope="{ node, data }" class="custom-tree-node" @click.stop="handleNodeClick(data)">
                    <div>
                      ...
                      <span v-if="data.space_property_name" style="font-size: 11px; color: #3889b1">{{ data.name }}</span>
                    </div>
                    <span class="el-ic">
                      <el-dropdown>
                        ...
                        <el-dropdown-menu slot="dropdown">
                          ...
                          <el-dropdown-item v-if="node.label !== '全部图片'"
                                            @click.native="onEdit(data.id)">编辑分类
                          </el-dropdown-item>
                          <el-dropdown-item v-if="node.label !== '全部图片'"
                                            @click.native="handleDelete(data.id)">删除分类
                          </el-dropdown-item>
                        </el-dropdown-menu>
                      </el-dropdown>
                    </span>
      ...
      <el-col v-bind="grid2" class="colLeft">
        <div class="conter mb15 relative">
          <div class="bnt">
            <el-button v-if="!pictureType" size="small" type="primary" class="mr15 mb20" @click="checkPics">使用选中图片</el-button>
            <div class="mb20">
              <el-tooltip class="item" effect="dark" content="上传图片" placement="top-start">
                <el-upload class="upload-demo" action :http-request="handleUploadForm" :on-change="imgSaveToUrl"
              ...
              <el-tooltip class="item" effect="dark" content="删除图片" placement="top-start">
                <el-button icon="el-icon-delete" class="mr15" type="danger" size="small" @click.stop="editPicList('图片')"
                  v-hasPermi="['admin:system:attachment:delete']" v-if="!pictureType"></el-button>
              </el-tooltip>
            </div>
            <el-upload class="upload-demo" action :http-request="handleUploadForm" :on-change="imgSaveToUrl"
               :show-file-list="false" multiple>
            ...
            <div>
              <el-button class="mr10" type="danger" @click.stop="editPicList('图片')" v-if="pictureType">删除图片</el-button>
            </div>
            <el-select
              v-model="sleOptions.attachment_category_name" placeholder="图片移动至" class="mb20" :size="pictureType ? '' : 'small'">
              <el-option class="demo" :label="sleOptions.attachment_category_name" :value="sleOptions.attachment_category_id" style="
/*<!--                  max-width: 560px;-->*/
/*<!--                  height: 200px;-->*/
/*<!--                  overflow: auto;-->*/
          ...
          <div class="pictrueList acea-row" v-loading="loadingPic">
            ...
            <div class="conters scrollbarAll" :style="{ maxHeight: !pictureType ? '500px' : '700px' }">
              <div v-for="(item, index) in pictrueList.list" :key="index" class="gridPic">
                ...
                <img style="object-fit: contain;" v-lazy="item.sattDir ? item.sattDir : localImg"
                  :class="item.isSelect ? 'on' : ''" @click="changImage(item, index, pictrueList.list)" v-if="item.attType !== 'video/mp4'"/>
                <video :src="item.sattDir" :class="item.isSelect ? 'on' : ''" @click="changImage(item, index, pictrueList.list)" v-if="item.attType == 'video/mp4'"></video>
              </div>
            </div>
          </div>
        </div>
        <div class="block">
          <el-pagination :page-sizes="!pictureType ? [10, 20, 30, 40] : [30, 60, 90, 120]" :page-size="tableData.limit"
            :current-page="tableData.page" :pager-count="5" layout="total, sizes, prev, pager, next, jumper"
            :total="pictrueList.total" @size-change="handleSizeChange" @current-change="pageChange"/>
        </div>
      </el-col>
    </el-row>
    <el-dialog
...
<script>
import {addCategroy, listCategroy, treeCategroy, infoCategroy, updateCategroy, deleteCategroy } from "@/api/system/category";
import {fileImageApi,fileListApi,fileDeleteApi,attachmentMoveApi} from "@/api/system/attachment";
export default {
  name: "Upload",
  props: {
    pictureType: {
