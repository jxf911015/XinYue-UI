<template>
  <div class="login">
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form">
      ...
      <el-form-item prop="code" v-if="captchaOnOff && !isSmsLogin">
        <el-input
...
<!--      <el-checkbox v-model="loginForm.rememberMe" style="margin:0px 0px 25px 0px;">记住密码</el-checkbox>-->
    ...
    <div class="el-login-footer">
      <span>Copyright © 2018-2022 xinyue All Rights Reserved.</span>
    </div>
  </div>
</template>

<script>
import { getCodeImg ,getSmsCodes} from "@/api/login";
    ...
    getSmsCode(){
      if (!this.computeTime) {
        this.$refs.loginForm.validate(valid => {
          if (valid) {
            getSmsCodes(this.loginForm.mobile).then(res =>{
              if(res.code === 200){
                this.$message({
                  message: '验证码已发送',
                  type: 'success'
