<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="bannerList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="地址" align="center" prop="imgUrl" />
      <el-table-column label="类型" align="center" prop="type">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.banner_type" :value="scope.row.type"/>
        </template>
      </el-table-column>
      <el-table-column label="跳转" align="center" prop="targetType">
        <template slot-scope="scope">
      ...
      <el-table-column label="是否显示" align="center" prop="isShow" width="100">
        <template slot-scope="scope">
          <image-preview :src="scope.row.isShow" :width="50" :height="50"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="图片" prop="imgId">
          <el-input v-model="form.imgId" placeholder="请输入图片" />
        </el-form-item>
        <el-form-item label="类型" prop="type">
...
:value="parseInt(dict.value)"
        ...
        <el-form-item label="跳转" prop="targetType">
...
:value="parseInt(dict.value)"
        ...
        <el-form-item label="是否显示">
          <image-upload v-model="form.isShow"/>
        </el-form-item>
        <el-form-item label="排序" prop="sort">
          <el-input v-model="form.sort" placeholder="请输入排序" />
        </el-form-item>
        <el-form-item label="备注" prop="remark">
          <el-input v-model="form.remark" placeholder="请输入排序" />
        </el-form-item>
      </el-form>
...
export default {
  name: "Banner",
  dicts: ['banner_target_type', 'banner_type'],
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        ...
        imgUrl: [
          { required: true, message: "焦点图地址不能为空", trigger: "blur" }
        ],
        type: [
          { required: true, message: "类型：1-top 2-banner不能为空", trigger: "change" }
        ],
        sort: [
          { required: true, message: "排序不能为空", trigger: "blur" }
        ],
        isShow: [
          { required: true, message: "是否显示：1-不能为空", trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除banner列编号为"' + ids + '"的数据项？').then(function() {
